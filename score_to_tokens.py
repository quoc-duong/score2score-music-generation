...
def attribute_to_token(child):  # clef, key signature, and time signature
    ...
    if type_ == 'clef':
        ...
        times = [int(c.text) for c in child.contents if isinstance(
            c, Tag)]  # excluding '\n'
        if times[1] == 2:
...
def note_to_tokens(note, divisions=8, note_name=True):  # notes and rests
    beam_translations = {'begin': 'start', 'end': 'stop',
                         'forward hook': 'partial-right', 'backward hook': 'partial-left'}
    ...
    for pitch in note.find_all('pitch'):
        if note_name:
            if pitch.alter:
                alter_to_symbol = {'-2': 'bb', '-1': 'b',
                                   '0': '', '1': '#', '2': '##'}
                tokens.append(
                    f"note_{pitch.step.text}{alter_to_symbol[pitch.alter.text]}{pitch.octave.text}")
            else:
                tokens.append(f"note_{pitch.step.text}{pitch.octave.text}")
        else:
            note_number = pretty_midi.note_name_to_number(
                pitch.step.text + pitch.octave.text)  # 'C4' -> 60
            if pitch.alter:
    ...
    if note.beam:
        ...
        tokens.append('beam_' + '_'.join([beam_translations[b.text]
                      if b.text in beam_translations else b.text for b in beams]))
...
def element_segmentation(measure, soup, staff=None):
    ...
    for element in measure.contents:
        if element.name == 'note':
            ...
            if element.staff and int(element.staff.text) == staff:
                voice_starts[voice] = min(
                    voice_starts[voice], position) if voice in voice_starts else position
            ...
            if element.staff and int(element.staff.text) == staff:
                voice_ends[voice] = max(
                    voice_ends[voice], position) if voice in voice_ends else position
    ...
    voice_end = sorted(voice_ends.values(), reverse=True)[
        1] if voice_ends else 0
...
def measures_to_tokens(measures, soup, staff=None, note_name=True):
    ...
    for measure in measures:
        ...
        tokens.append('bar')
        if staff is not None:
            notes = [n for n in measure.find_all(
                'note') if n.staff and int(n.staff.text) == staff]
        else:
        ...
        for voice in voices:
            voice_notes = [
                n for n in notes if n.voice and n.voice.text == voice]
            aggregate_notes(voice_notes)

        if len(voices) > 1:
            pre_voice_elements, voice_elements, post_voice_elements = element_segmentation(
                measure, soup, staff)
                ...
                for voice in voices:
                    ...
                    for element in voice_elements:
                        if (element.voice and element.voice.text == voice) or (not element.voice and voice == '1'):
                            if element.name == 'attributes':
                                attr_tokens, div = attributes_to_tokens(
                                    element, staff)
                                tokens += attr_tokens
                                divisions = div if div else divisions
                            elif element.name == 'note':
                                tokens += note_to_tokens(element,
                                                         divisions, note_name)
                    tokens.append('</voice>')
...
def load_MusicXML(mxml_path):  # load MusicXML contents using BeautifulSoup
    soup = BeautifulSoup(open(mxml_path, encoding='utf-8'),
                         'lxml-xml', from_encoding='utf-8')  # MusicXML
